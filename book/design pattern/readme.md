...
### [橋接模式 (Bridge)](./bridge.md)  
將抽象與實現分離，使兩者可以獨立變化。  
示例：圖形API與具體繪圖實現的解耦。

### [組合模式 (Composite)](./composite.md)
將物件組合成樹形結構，表示「部分-整體」的層次關係。  
示例：文件系統中的目錄與文件。

### [裝飾器模式 (Decorator)](./decorator.md)  
動態地為物件添加額外的職責。  
示例：為IO流添加加密或壓縮功能。

### [外觀模式 (Facade)](./facade.md)  
為複雜子系統提供一個簡單的接口。  
示例：為多模組系統提供統一入口。
...
### [解釋器模式 (Interpreter)](./interpreter.md)  
為特定語言定義語法表示，並實現解釋器。  
示例：正則表達式解析。
